...
enum class FightingFantasyGamebook(
    ...
    companion object {
        ...
        private val initialsMatrix = values().map { it.initials to it }.toMap()
        private val numberMatrix = values().map { it.order to it }.toMap()

        fun gamebookFromInitials(initials: String) = initialsMatrix[initials]
