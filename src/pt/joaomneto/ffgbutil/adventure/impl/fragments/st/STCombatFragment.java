	...
	private void phaserCombatTurn() {
		...
		if (!finishedCombats.contains(currentCombat)) {
			...
			if (!position.isDefenseOnly()) {
				...
				if (position.getCurrentStamina() > 0 && enemyDiceRoll < position.getCurrentSkill()) {
					adv.setCrewmanDead(position.getCrewman());
					combatResult.setText("The " + crewmanString + " has died... (" + enemyDiceRoll + ") > ("
							+ position.getCurrentSkill() + ")");
					row.removeAllViews();
					finishedCombats.add(currentCombat);
				} else {
					combatResult.setText("Both enemy has missed his shot!");
				}
			}else{
				if (position.getCurrentStamina() > 0 && enemyDiceRoll < position.getCurrentSkill()) {
					...
					combatResult.setText("The " + adv.getStringForCrewman(killedCrewmanObj) + " has died... (" + enemyDiceRoll + ") > ("
							+ position.getCurrentSkill() + ")");
					row.removeAllViews();
					finishedCombats.add(currentCombat);
				} else {
					combatResult.setText("The enemy (Sk:"+position.getCurrentSkill()+" St:"+position.getCurrentStamina()+" ) has missed his shot!");
				}
			}
